...
export async function POST(request: NextRequest) {
  ...
  try {
        ...
        const accessToken = await getAccessToken(refreshToken);
          const event = await createCalendarEvent(accessToken, title, date, startTime, endTime, timezone, location);

        return NextResponse.json({ ok: true, htmlLink: event.htmlLink, eventId: event.id });
  } catch (err) {
