...
interface ExtractedData {
  ...
  startTime: string; // HH:mm (24h) for <input type="time">
...
export default function Home() {
  ...
  const checkAuthStatus = async () => {
    ...
    // If we came back from OAuth with a success flag, re-check auth status fresh
    ...
    // Normal check
    try {
      const r = await fetch("/api/auth/status", { cache: "no-store" as RequestCache });
  ...
  const connectGoogle = async () => {
    ...
    try {
      const r = await fetch("/api/auth/google/start", { cache: "no-store" as RequestCache });
      const d = await r.json();
      if (d?.url) {
        window.location.href = d.url;
      } else {
        setStatus({ type: "error", message: "No auth URL returned." });
      }
    } catch {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          let final: Partial<ExtractedData> = { ...parsed };

          // Low confidence → optional Gemini fallback
          if (confidence < 50) {
            setStatus({ type: "info", message: "Low confidence — trying AI fallback..." });
            try {
              const gemini = await parseWithGemini(text);
              if (gemini) final = { ...final, ...gemini };
            } catch {
              // If Gemini fails, keep deterministic results
            }
          }

          setExtracted({
            title: (final.title as string) || "",
            date: (final.date as string) || "",
            startTime: (final.startTime as string) || "",
            endTime: (final.endTime as string) || "",
            timezone: (final.timezone as string) || "America/Chicago",
            location: (final.location as string) || "",
            category: (final.category as Category) || CATEGORIES[0],
            extractedText: text,
          });

          setStatus({ type: "success", message: "OCR completed. Review and edit if needed." });
        } catch {
          setStatus({ type: "error", message: "Failed to process image text (OCR error)." });
        } finally {
          setLoadingOCR(false);
        }
      };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (f.length > 0) void handleFileSelect(f[0]);
  };
  ...
  const handleInputChange = (
    ...
    setExtracted((prev) => ({ ...prev, [name]: value }));
  };
  ...
  const createCalendarEvent = async () => {
    ...
    if (!extracted.date) {
      setStatus({ type: "error", message: "Please enter a date." });
      return;
    }
    if (!extracted.startTime) {
      setStatus({ type: "error", message: "Please enter a start time." });
      return;
    }
    ...
    try {
      ...
      if (r.ok) {
        setStatus({ type: "success", message: "✅ Calendar event created." });
      } else {
  ...
  const logToSheet = async () => {
    ...
    try {
      ...
      if (r.ok) {
        setStatus({ type: "success", message: "✅ Logged to Google Sheet." });
      } else {
      ...
      {!screenshot ? (
        <div className="upload-section">
          ...
          >
            ...
            <span className="file-input-label">{loadingOCR ? "Working..." : "Choose Image"}</span>
          </label>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // reset so picking same file again still triggers
        ...
        <div className="preview-section">
          ...
          <div className="extraction-form">
            {extracted.extractedText && (
              <div className="intent-badge">Intent: {extracted.category}</div>
            )}
            ...
            {!isAuthenticated ? (
              ...
              <div className="button-group">
                <button className="btn-primary" onClick={createCalendarEvent} disabled={loadingCalendar}>
                  {loadingCalendar ? <span className="loading" /> : null}
                  Calendar
                </button>
                <button className="btn-primary" onClick={logToSheet} disabled={loadingSheet}>
                  {loadingSheet ? <span className="loading" /> : null}
                  Log Sheet
                </button>
              </div>
            )}
            ...
            <button
              ...
              onClick={startOver}
              disabled={loadingCalendar || loadingSheet || loadingOCR}
            >
              Start Over
            </button>
          </div>
