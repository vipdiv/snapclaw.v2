...
export default function Home() {
  ...
  const createCalendarEvent = async () => {
        ...
        try {
                ...
                const d = await r.json();
                setStatus(r.ok ? { type: "success", message: "Event created successfully" } : { type: "error", message: `Calendar error: ${d.error || "Failed"}` });
        } catch { setStatus({ type: "error", message: "Failed to create event" }); }
  ...
  const logToSheet = async () => {
        ...
        try {
                ...
                const d = await r.json();
                setStatus(r.ok ? { type: "success", message: "Logged to sheet successfully" } : { type: "error", message: `Sheet error: ${d.error || "Failed"}` });
        } catch { setStatus({ type: "error", message: "Failed to log to sheet" }); }
  ...
  return (
        <div className="container">
          ...
          {status && <div className={`status-message ${status.type}`}>{status.message}</div>div>}
          {!screenshot ? (
